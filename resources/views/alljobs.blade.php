                        ...
                        <div class="col-lg-3">
                            <div class="filter-secs">
                                ...
                                <div class="paddy" style="padding-bottom:15px">
                                    <form method="get" action="{{url('alljobs')}}">
                                        <div class="form-group">
                                            <label for="exampleInputEmail1">Name</label>
                                            <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Job Name" name="title" @if(isset($_GET["title"])) value="{{$_GET["title"]}}" @endif>
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputPassword1">Jurusan</label>
                                            <select class="form-control" id="exampleInputPassword1" name="jurusan">
                                                <option value="all" @if(isset($_GET["jurusan"])&&$_GET["jurusan"]=="semua") selected @endif>All</option>
                        ...
                        <div class="col-lg-6">
                            <div class="main-ws-sec">
                                ...
                                <div class="post-topbar">
                                    ...
                                    <div class="post-st">
                                        <ul>
                                            <li><a class="active" href="{{url('postjob')}}" title="">Post a Job</a></li>
                                        </ul>
                                    </div><!--post-st end-->
                                </div><!--post-topbar end-->
                                @endif
                                <div class="posts-section" id="job">
                                    <?php 
                                        $querystring = "";
                                        if(isset($_GET["title"]))
                                        {
                                        ...
                                        if(isset($_GET["jurusan"])&&$_GET["jurusan"]!="all")
                                        {
                                            ...
                                            $thisjobs = App\Job::whereHas('jurusans',function($request)use($jurusan){
                                                $request->where('jurusan_id',$jurusan);
                                            })->where('name','like','%'.$namesearch.'%')->orderBy('id','desc')->paginate('4');
                                            if($thisjobs->count()>0)
                                            {
                                                $firstjob= App\Job::whereHas('jurusans',function($request)use($jurusan){
                                                    $request->where('jurusan_id',$jurusan);
                                        ...
                                        {
                                            $thisjobs = App\Job::where('name','like','%'.$namesearch.'%')->orderBy('id','desc')->paginate('4');
                                            if($thisjobs->count()>0)
                                            {
                                                $firstjob= App\Job::where('name','like','%'.$namesearch.'%')->first()->id;
                                            }
                                            else
                                            {
                                                $firstjob = 0;
                                        ...
                                        <div class="post-bar">
                                            <div class="post_topbar">
                                                <div class="usy-dt">
                                                    ...
                                                    <div class="usy-name">
                                                        <a href="{{url('/viewprofile/'.$job->user->nickname)}}"><h3>{{$job->user->nickname}}</h3></a>
                                            ...
                                            <div class="job_descp">
                                                ...
                                                <p>{!!substr(strip_tags(nl2br($job->text),'<br>'),0,200)!!}...<a href="{{url('/viewjob/'.$job->slug)}}" title="">view more</a></p>
                                                <ul class="skill-tags">
...
@section('js')
    <script>
        ...
        console.log("{{url('/getjob')}}/"+job_id+querystring);
        function morearticle() 
        {
            $( ".process-comm" ).remove();
            $.ajax({
                url: "{{url('/getjob')}}/"+job_id+querystring,
                success: function (data) { $('#job').append(data); },
                dataType: 'html'
            });
        }
    </script>
