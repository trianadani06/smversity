...
?>
@foreach(App\Status::where('id','<',$stat_id)->where('ukm_id',$ukm_id)->orderBy('created_at','desc')->paginate(4) as $status)
    ...
    @endforeach
    @if(App\Status::where('id','<',$stat_id)->where('ukm_id',$ukm_id)->orderBy('created_at','desc')->count() > 0)
        <div class="process-comm" id="process-status">
            <a onClick="moreukmstatus()" title=""><img src="images/process-icon.png" alt=""></a>
